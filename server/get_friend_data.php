...
function get_news_data(){
	...
	for($a=1;$a<=count($albumdata);$a++){
		for($b=1;$b<=(count($albumdata[$a-1]))-4;$b++){
			$getpicturedata= mysql_query("SELECT `content`,`attraction_id`,`purl` FROM `picture` WHERE `id`=".$albumdata[$a-1][$b][0]."");
			...
			for($c=1;$c<=mysql_num_rows($getpicturedata);$c++){ 
				...
				for($d=1;$d<=mysql_num_rows($getattractionname);$d++){ 
					...
					$albumdata[$a-1][$b] = array("1" => $row[0], "2" => $row[1], "3" => $rs[0], "4" => $row[2]);
				}
			}
		}
	}
